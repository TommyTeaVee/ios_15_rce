...
echo "Double Tap Home Button"

vibrate=$(timelimit -q -t4 idevicesyslog | grep -o "AppleSPUButtonDriver: 0\|HomeButtonSinglePressUp\|Menu Button Pressed\|Home usagePage:0xc usage:0x40 downEvent:0 up\|source: HomeButton" | wc -l)


if [ ${vibrate} -ge 1 ]; then
	echo "Home Button Pass"
else
	echo "Home Button Fail"
fi
