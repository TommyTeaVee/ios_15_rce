...
vibrate=$(timelimit -q -t3 idevicesyslog | grep -o "ActiveCategory_IncrementBy, value = -0.062\|Volume Lower\|VolumeControl -- volume decrement -- button state: up\|VolumeControl -- volume decrement down:1" | wc -l)

if [ ${vibrate} -ge 1 ]; then
	echo "Volume Down Pass"
else
	echo "Volume Down Fail"
fi
