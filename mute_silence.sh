...
echo "Toggle the mute/silence switch"

vibrate=$(timelimit -q -t4 idevicesyslog | grep -o "Ringer switch state: on\|Ringer switch state: muted" | wc -l)

if [ ${vibrate} -ge 1 ]; then
	echo "Mute/Silence Pass"
else
	echo "Mute/Silence Fail"
fi
